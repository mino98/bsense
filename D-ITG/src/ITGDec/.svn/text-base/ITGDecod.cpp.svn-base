...
char bitratefile[fileNameSize] = "bitrate.txt";
char jitterfile[fileNameSize] = "jitter.txt";
char pktlossfile[fileNameSize] = "packetloss.txt";
char delayfile[fileNameSize] = "delay.txt";
char combinedstatsfile[fileNameSize] = "combinedstats.txt";
...
#endif
		fprintf(outtxt, "txTime>%ld:%ld:%lf ", (*infos).txTime1, (*infos).txTime2, (*infos).txTime3);
		fprintf(outtxt, "rxTime>%ld:%ld:%lf ", (*infos).rxTime1, (*infos).rxTime2, (*infos).rxTime3);
...
void setline(double txTime, double rxTime)
{
	...
	if (flownum == 0) { 
		...
		PRINTD(2,"Flow=%d Seq=%d pktrecv=%ld pktloss=%ld pktdup=%d\n",
...
void merge(int type)
{
	...
	case 1:
		...
		if ((fpout = fopen(bitratefile, "w")) == NULL) {
			printf("Error opening file 'bitrate.txt' for write");
			perror("error ");
			exit(1);
		}
		break;
	case 2:
		...
		if ((fpout = fopen(jitterfile, "w")) == NULL) {
			printf("Error opening file 'jitter.txt' for write");
			perror("error ");
			exit(1);
		}
		break;
	case 3:
		...
		if ((fpout = fopen(delayfile, "w")) == NULL) {
			printf("Error opening file 'delay.txt' for write");
			perror("error ");
			exit(1);
		}
		break;
	case 4:
		...
		if ((fpout = fopen(pktlossfile, "w")) == NULL) {
			printf("Error opening file 'pktloss.txt' for write");
			perror("error ");
			exit(1);
		}
		break;
...
void mergeCombined() {
	...
	// Rows in the output files are in the form:
	// "Time Bitrate Delay Jitter PacketLoss"
...
void printhelp()
{
	...
	printf("          [-p <packetloss_interval_size>][-c <combined_interval_size>]\n");
	printf("          [-f <max_flow_num>]]|[-h|--help]\n\n");
	printf(" Options\n");
	printf(" -d <DT> [filename] generate file with average delay every <DT> milliseconds\n");
	printf("                    (default filename: 'delay.txt')\n\n");
	printf(" -j <JT> [filename] generate file with average jitter every <JT> milliseconds\n");
	printf("                    (default filename: 'jitter.txt')\n\n");
	printf(" -b <BT> [filename] generate  file with average bitrate every <BT> milliseconds\n");
	printf("                    (default filename: 'bitrate.txt')\n\n");
	printf(" -p <PT> [filename] generate file with average packet loss every <PT> milliseconds\n");
	printf("                    (default filename: 'packetloss.txt')\n\n");
	printf(" -c <CT> [filename] generate file with combined statistics every <CT> milliseconds\n");
	printf("                    (default filename: 'combinedstats.txt')\n\n");
	...
	printf(" -P                 print on screen the size of each packet\n\n");
	printf(" -I                 print on screen the inter departure time between each packet\n\n");
	
	
	
	printf("\n For more information refer to the help file in the home directory\n");
}

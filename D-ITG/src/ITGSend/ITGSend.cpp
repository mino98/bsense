	...
	while (argc > 0) {
		if (argv[0][0] == '-') {
			...
			case 'L':
				...
				exit(-1);
			case 'X':
				printf("Option '-X' is disabled in this version, part of the BSense project\n");
				exit(-1);
			case 'x':
				printf("Option '-x' is disabled in this version, part of the BSense project\n");
				exit(-1);
...
void *flowParser(void *param)
{
	...
	getaddrinfo(DefaultDestIP, NULL, NULL, &flows[id].SigDestHost);				
		...
		if (argv[h][0] == '-') {
			...
			case 'p':
				if (argv[h][2] != 'o'){
					...
					PRINTD(1,"Polling mode for flow n° %d\n",id);
			...
			case 'a':
			{
				...
				if (SigDestHostSpecify == false){
					...
					if ((argc < 2) || getaddrinfo(argv[h + 1], NULL, NULL, &flows[id].SigDestHost))
						ReportErrorAndExit("General parser","Invalid destination address for the signal channel", programName, id);

#ifndef NOIPV6
					if (flows[id].SigDestHost->ai_family == PF_INET6 &&
							IN6_IS_ADDR_LINKLOCAL(((struct sockaddr_in6 *)(flows[id].SigDestHost->ai_addr))->sin6_addr.s6_addr32) &&
			...
			case 'T':
				...
				if (flows[id].l4Proto == L4_PROTO_SCTP) {
					...
					if (argv[h + 3][0] > '0' || argv[h + 3][0] < '9')
				  	...
				  	flows[id].minPayloadSize = flows[id].minPayloadSize + sizeof(int);
					PRINTD(1, "in sctp settato flows[id].minPayloadSize: %d\n", flows[id].minPayloadSize);
			...
			case 's':
				if (strlen(argv[h])==2){
			    	...
			    	if(flows[id].iface == NULL){
						...
						if ((argc < 2) || getaddrinfo(argv[h + 1], NULL, NULL, &flows[id].SrcHost ))
							ReportErrorAndExit("General parser", "Invalid source address",programName, id);
#ifndef NOIPV6
						if (flows[id].SrcHost->ai_family == PF_INET6 &&
							IN6_IS_ADDR_LINKLOCAL(((struct sockaddr_in6 *)(flows[id].SrcHost->ai_addr))->sin6_addr.s6_addr32) &&
			...
			case 'F': 
				...
				if (argv[h][2] == 's') {
					...
					PRINTD(1,"File name of the packets sizes: %s\n",flows[id].pktSizeFile);
				...
				if (argv[h][2] == 't') {
					...
					PRINTD(1,"File name of the IDT: %s\n",flows[id].timeFile);
			...
			case 'S': 
				if (argv[h][2] == 's') { 
                    if (argv[h][3] == 'a') { 
                    	...
                    	if ((argc < 2) || getaddrinfo(argv[h + 1], NULL, NULL, &flows[id].SigSrcHost))
                    		ReportErrorAndExit("General parser","Invalid source address for the signal channel", programName, id);

#ifndef NOIPV6
                    	if (flows[id].SigSrcHost->ai_family == PF_INET6 &&
                    			IN6_IS_ADDR_LINKLOCAL(((struct sockaddr_in6 *)(flows[id].SigSrcHost->ai_addr))->sin6_addr.s6_addr32) &&
                    			((struct sockaddr_in6 *)(flows[id].SigSrcHost->ai_addr))->sin6_scope_id == 0)
                    		printf("** WARNING **: IPv6 address scope id not specified by the sender (signal channel)\n");
#endif

                    	PRINTD(1,"\nSignaling source address: %s\n",argv[h + 1]);
                    }else if (argv[h][3] == 'p') { 
                    	if (argc < 2)
                    		ReportErrorAndExit("General parser", "Invalid port number", programName, id);
					...
					if (argv[h][3] == 'a') { 
						...
						PRINTD(1,"\nSignaling destination address: %s\n",argv[h + 1]);
						SET_PORT(flows[id].SigDestHost,SigPort);
					}else if (argv[h][3] == 'p') { 
						if (argc < 2)
							ReportErrorAndExit("General parser", "Invalid port number", programName, id);
	...
	if(flag!=flows[id].dstAddrSpecify){
		...
		if(getaddrinfo(buffer, NULL, NULL,&(flows[id].DestHost)) < 0){
	...
	if((passiveMode==true) && (flows[id].srcAddrSpecify==false)){
		if (flows[id].SigDestHost->ai_family == PF_INET6){
			...
			if (flows[id].SrcHost) freeaddrinfo(flows[id].SrcHost);
			if(getaddrinfo("::", NULL, NULL,&(flows[id].SrcHost)) < 0){
				perror("flowParser getaddrinfo() (2)");
				exitThread();
			}

#ifndef NOIPV6
			if (IN6_IS_ADDR_LINKLOCAL(((struct sockaddr_in6 *)(flows[id].SrcHost->ai_addr))->sin6_addr.s6_addr32) &&
					((struct sockaddr_in6 *)(flows[id].SrcHost->ai_addr))->sin6_scope_id == 0)
				printf("** WARNING **: IPv6 address scope id not specified by the sender\n");
#endif
#endif
	...
	for (;;) {
		...
		while(available > 0) {
			...
			case MSG_SM_ERRFLOW:
				...
				PRINTD(1,"recover from a receiver error notification \n");
	...
	if ((checkChanID == false) && (okToNewSignalingChan == true)) {												
		...
		*chanId = 0;
		...
		if ((passiveMode == true) && (multiFlows == 1)){
			...
			if(getaddrinfo(globalSigAddr, NULL, NULL,&(DestHost)) < 0){
...
void *flowSender(void *para)
{
	...
	if (DestHost->ai_family == PF_INET6){
		getaddrinfo("::", NULL, NULL, &LogAddr);
	}else if (DestHost->ai_family == PF_INET){
		getaddrinfo("0.0.0.0", NULL, NULL, &LogAddr);
	}
	...
	if (dimVectSize > 0){
		SizeFile=true;
		indexVectSize=0;
		PRINTD(1,"flows[id].vectSize.size() = %d\n",dimVectSize);
	}
	...
	if (dimVectTime > 0){
		TimeFile=true;
		indexVectTime=1;
		PRINTD(1,"fflows[id].vectTime.size() = %d\n",dimVectTime);
	}
	

	
	Uniform * randomVariable = new Uniform;
	for (int i=0; i<MAX_PAYLOAD_SIZE;i++)
		payload[i]=(unsigned char)(255*randomVariable->Next());
	...
	if (getaddrinfo(dotIpV6, NULL, NULL, &SrcHost) < 0)  {		
		
		PRINTD(1,"Try to create an IPv4 Socket\n");
		if (SrcHost) freeaddrinfo(SrcHost);
		
		if (getaddrinfo(dotIpV4, NULL, NULL, &SrcHost) < 0) {		
			perror("flowSender");
	...
	if ((l7Proto != L7_PROTO_TELNET && l7Proto != L7_PROTO_DNS) && socktype == SOCK_STREAM
		...
		if (setsockopt(sock, getprotobyname("TCP")->p_proto, TCP_NODELAY, (char *)no_delayWin,
	...
	if(passiveMode == false ){				
		...
		if (flows[id].l4Proto == L4_PROTO_TCP){
			...
			PRINTD(1,"flowSender in listening. (Passive Mode)\n");
		}
	}
	...
	if (l4Proto == L4_PROTO_TCP) {
		...
		if (flows[id].payloadLogType == PL_STANDARD)
			...
			ptrSize = payload + sizeof(uint32_t);
	}
	...
	if ((Duration == DefaultDuration) && (numPacket > 0)){
		Duration=ULONG_MAX;
		PRINTD(1,"Number of packets to generate : %d \n",numPacket);
	}else if(numPacket == 0){
		numPacket=UINT_MAX;
	}
	...
	if ((Duration == DefaultDuration) && (byteToSend > 0)){
		Duration=ULONG_MAX;
		PRINTD(1,"Number of Kbyte to generate : %lu \n",flows[id].KBToSend);
	...
	PRINTD(1,"Size for first packet %d\n",size);
	...
	do {
		...
		uint32_t net_TimeSec = htonl(time);
		uint32_t net_TimeUsec = htonl(Ticker.lastTime.tv_usec);
		uint32_t net_size = htonl(size);
		

		
		if ( flows[id].payloadLogType == 0 ) { 
			memcpy(ptrSeqNum, &net_seqNum, sizeof(uint32_t));
			memcpy(ptrTimeSec, &net_TimeSec, sizeof(uint32_t));
			memcpy(ptrTimeUsec, &net_TimeUsec, sizeof(uint32_t));
		...
		if (l4Proto == L4_PROTO_TCP)
#endif
			memcpy(ptrSize, &net_size, sizeof(uint32_t));
...
// Let's silence this message, unless we're in debug mode:
#if DEBUG
			...
			if ((logging) && (meter == METER_OWDM)) {
				...
				if ( logremoto == 0) {
					...
					writeInBufferStandard(&infos[count], htonl(id), htonl(seqNum), HelpSrcAddress, HelpDstAddress, tmpPort_SrcPort, tmpPort_DstPort, time, time, Ticker.lastTime.tv_usec, Ticker.lastTime.tv_usec,size);
				} else {

					writeInBufferStandard(&infos[count],*(unsigned int *) payload ,*(unsigned int *) ptrSeqNum, HelpSrcAddress, HelpDstAddress, tmpPort_SrcPort,tmpPort_DstPort, 0,0,0,0,size);	
					infos[count].srcPort = htonl(infos[count].srcPort);
					infos[count].destPort = htonl(infos[count].destPort);
					
					infos[count].txTime1 = net_TimeSec;
					
					
					infos[count].txTime2 = net_TimeUsec;
					infos[count].txTime3 = 0;
					
					infos[count].rxTime1 = net_TimeSec;
					
					
					infos[count].rxTime2 = net_TimeUsec;
					infos[count].rxTime3 = 0;
					infos[count].size = htonl(infos[count].size);           
					infos[count].flowId = htonl(infos[count].flowId);       
					infos[count].seqNum = htonl(infos[count].seqNum);       
				}
		...
		PRINTD(2,"Size for next packet %d\n",size);
		...
		PRINTD(2,"IDT for next packet %f ms\n",wait);
				...
				if (FD_ISSET(sock, &read_set)) {
					...
					if (((logging) && (size_r > 0)) || (meter == METER_RTTM)) {
						if (logremoto == 0) {
				            ...
				            writeInBufferStandard(&infos[count],*(unsigned int *) payload,*(unsigned int *) ptrSeqNum,HelpSrcAddress, HelpDstAddress, tmpPort_SrcPort, tmpPort_DstPort, 0,0,0,0,htonl(size_r));
				            infos[count].srcPort = htonl(infos[count].srcPort);
				            infos[count].destPort = htonl(infos[count].destPort);
	...
	} while ((Ticker.lastTime.tv_sec < end_time.tv_sec || (Ticker.lastTime.tv_sec == end_time.tv_sec && Ticker.lastTime.tv_usec < end_time.tv_usec)) &&
...
void printHelp()
{
	...
	cout << "   - Otherwise:" << endl;
	cout << "   ./ITGSend [-H] [-m <msr_type>][-a <destination_address>] [-sa <source_address>] " <<endl;
	cout << "      [-sp <source_port>] [-rp <destination_port>]"<<endl;
	cout << "      [-Sda <signaling_dest_addr>] [-Sdp <signaling_dest_port>]" << endl;
	cout << "      [-Ssa <signaling_src_addr>] [-Ssp <signaling_src_port>]" << endl;
	...
	cout << "      [-p <payload_log_type] "<<endl;
	cout << "      [-Fs <file_name_of_sizes>] [-Ft <file_name_of_IDT>]"<<endl;		
	cout << "      [-z <number_of_packets>] [-k <number_of_KBytes>] [-poll]";
#ifdef WIN32
    cout << " [-P] ";
#endif
#ifdef LINUX_OS
    cout << endl;
    cout << "      [-i <interface>] [-Si <interface>]" << endl ;
#endif
	...
	cout << "  -H\t\t\t\t\tPassive Mode (please, for details see the documentation)"<<endl;
	...
	cout << "  -sa  <source_address>\t\t\tSet the source address\n\t\t\t\t\t\(Default: Set by O.S.)" << endl;  
	cout << "  -rp <destination_port>\t\tSet the destination port \n\t\t\t\t\t\(Default: " << DEFAULT_PORT << ") " << endl;
	cout << "  -sp <source_port>\t\t\tSet the source port \n\t\t\t\t\t\(Default: Set by O.S.)" << endl;
	cout << "  -Sda  <signaling_dest_addr>\t\tSet the destination address for the signaling channel\n\t\t\t\t\t\(Default: " << DefaultDestIP << " or <destination_address>)" << endl;
	cout << "  -Sdp  <signaling_dest_port>\t\tSet the destination port for the signaling channel\n\t\t\t\t\t\(Default: " << DEFAULT_PORT_SIGNALING << ")" << endl;
	cout << "  -Ssa  <signaling_src_addr>\t\tSet the source address for the signaling channel" << endl;
	cout << "  -Ssp  <signaling_src_port>\t\tSet the source port for the signaling channel" << endl;
	...
	cout << "  -Si  <signaling_interface>\t\tSet the interface for the signaling channel" << endl;
#endif
	cout << "  -t  <duration>\t\t\tSet the generation duration\n\t\t\t\t\t(Default: " << DefaultDuration << " msecs)" << endl;
	cout << "  -z  <number_of_packets>\t\tNumber of packets to generate"<<endl;
	cout << "  -k  <number_of_KBytes>\t\tNumber of KBytes to generate"<<endl;
...
#endif
	cout << "  -poll \t\t\t\tPolling mode (please, for details see the documentation)" << endl;
	...
	cout << "  -Fs  <file_name_of_sizes>\t\tRead payloads sizes from file" << endl; 
	cout << "  -Ft  <file_name_of_IDT>\t\tRead IDT from file" << endl; 
...
#endif
{
...
#if defined LINUX_OS && ! defined OSX
	if (Interface!=NULL){
	...
	if (passiveMode==false){	
		...
		if(SrcHost!=NULL){
			...
			if (DestHost->ai_family == PF_INET6){
				if (SrcHost) freeaddrinfo(SrcHost);
				getaddrinfo("::", NULL, NULL,&SrcHost);
			}else
#endif
			{
				if (SrcHost) freeaddrinfo(SrcHost);
				getaddrinfo("0.0.0.0", NULL, NULL,&SrcHost);
			}
			SET_PORT(SrcHost,htons(SrcPort));
		}
		...
		if (SrcHost->ai_family == PF_INET6){
			...
			getaddrinfo("::", NULL, NULL,&TmpAddress);
			PRINTD(1,"createTransportChan : bind to IPv6 socket\n");
		} else
#endif
		{
			if (TmpAddress) freeaddrinfo(TmpAddress);
			getaddrinfo("0.0.0.0", NULL, NULL,&TmpAddress);
		...
		if (dstAddrSpecify == false){
			...
			if (DestHost) freeaddrinfo(DestHost);
				getaddrinfo(buffer, NULL, NULL,&DestHost);
			*DestHost=*TmpAddress;
...
int requestToSend(int flowId, int signalSock)
{
	...
	if (passiveMode==false)									
		...
		GET_PORT((flows[flowId].DestHost),port);			
	}
	...
	BYTE srcAddrSpecify;									
	if (flows[flowId].srcAddrSpecify == true)
		srcAddrSpecify=0xFF;
	else
		srcAddrSpecify=0x00;
	...
	next = putValue(next, (void *) &plType, sizeof(int));
 	...
 	if (passiveMode==true){															
 		next = putValue(next, (void *) &(port), sizeof(port));						
 		sizeMessag += sizeof(port);													

 		next = putValue(next, (void *) &(srcAddrSpecify), sizeof(srcAddrSpecify));	
 		sizeMessag += sizeof(srcAddrSpecify);										
 	}
	...
	if (DestHost->ai_family == PF_INET) {
		...
		PRINTD(1, "-- ScopeID = %lu --\n", (u_long)((struct sockaddr_in6 *) DestHost->ai_addr)->sin6_scope_id);

...
int mutexThreadInit(HANDLE &mutex)
{
	int ret = 0;
	 ...
	 PRINTD(2,"Return value CreateMutex (WIN32) : %d \n",(int)mutex);
     PRINTD(1,"Name mutex %s \n", nameMutex);
...
int mutexThreadInit(void* mutex)
{
	...
	PRINTD(2,"Return value pthread_mutex_init (LINUX) : %p \n", mutex);
	PRINTD(2,"Return value mutexThreadInit : %d \n",ret);
	return ret;
}
#endif
...
int mutexThreadUnlock(void *mutex)
{
    ...
    PRINTD(2,"Return value ReleaseMutex(WIN32) in mutexThreadUnlock : %d\n",flag);
	if (flag == 0) ret = -1;
#endif

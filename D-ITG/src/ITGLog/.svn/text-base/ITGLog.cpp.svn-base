...
void recvInfo(int signalingChannel, BYTE &protocol, char logFile[DIM_LOG_FILE])
{
	...
	PRINTD(1,"Sent LOG_CONNECT message\n");
	...
	PRINTD(1,"Received infos on log activity\n");
...
void createSignalingChannel()
{
	...
	PRINTD(1,"TCP signaling channel created with signalSocket : %d\n",signalSocket);
}
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1){
		if (strcmp(argv[1],"-gui") == 0){
			createThread((void*)"wait", &waitStopKey, NULL, t1);
			argc--;
			argv++;
		}
	}
	...
	while (1) {
		...
		PRINTD(2,"Return value CREATE_THREAD channelManager hThr[i] : %d \n",(int)hThr[i]);
		i++;
		}
	}
	return 0;

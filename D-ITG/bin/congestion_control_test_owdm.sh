...
do
	for i in {1..5}
	do
		./ITGSend -a $server -C $packetrate -c $packetsize -t $duration -T TCP -Sdp 3001 -rp 3005 -l congestion_owdm/${packetrate}_${i}.log > /dev/null 2> /dev/null

		achieved=`./ITGDec congestion_owdm/${packetrate}_${i}.log 2>/dev/null | grep "Average packet rate" | tail -1 | sed -e "s|.*= *\([^ ]*\).*|\1|"`
		rate=`./ITGDec congestion_owdm/${packetrate}_${i}.log 2>/dev/null | grep "Average bitrate" | tail -1 | sed -e "s|.*= *\([^ ]*\).*|\1|"`

...
do
	for i in {1..5}
	do
		./ITGSend -a $server -C $packetrate -c $packetsize -t $duration -T TCP -m rttm -Sdp 3001 -rp 3005 -l congestion_tcp/${packetrate}_${i}.log > /dev/null

		achieved=`./ITGDec congestion_tcp/${packetrate}_${i}.log 2>/dev/null | grep "Average packet rate" | tail -1 | sed -e "s|.*= *\([^ ]*\).*|\1|"`
		rate=`./ITGDec congestion_tcp/${packetrate}_${i}.log 2>/dev/null | grep "Average bitrate" | tail -1 | sed -e "s|.*= *\([^ ]*\).*|\1|"`
